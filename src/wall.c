...
/**
 * changeColorIntensity - Changes the intensity of a color based on a factor value between 0 and 1.
 * @factor: Intensity factor.
 * @color: Color for intensity adjustment.
 *
 * Description: This function adjusts the intensity of the given color by multiplying its RGB components
 ...
 *
 * Description: This function renders the floor projection for a given ray by calculating the distance
 ...
 * @x: Current element in the rays array.
 *
 * Description: This function renders the ceiling projection for a given ray by calculating the distance
...
void renderCeil(int wallTopPixel, color_t *texelColor, int x)
{
    ...
    for (y = wallTopPixel; y >= 0; y--)
    {
        ...
        float distance = (ratio * PROJ_PLANE) / cos(rays[x].rayAngle - player.rotationAngle);
 ...
 *
 * Description: This function renders the wall projection by calculating the height of the wall strip,
 * mapping the corresponding texture, and adjusting the intensity for shaded areas.
 *
 * Return: None.
 */
